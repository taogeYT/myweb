...
'''datetime.datetime 是一个时间类对象，用下面方式创建时间对象
datetime.datetime(2017, 3, 15, 23, 17, 30, 915153,tzinfo=tz0)	tz0代表时区对象
也可以通过datetime对象.replace(tzinfo= tz0）创建完时间对象之后在添加时区
datetime.timedelta(days=d,hours=h, minutes=m)
'''
class UTC(datetime.tzinfo): #定义时区对象类   用于生成时间时写入时区
    """
    ...
    >>> l1 = u.astimezone(tz8) 				#astimezone 时区转换
    >>> l2 = u.replace(tzinfo=tz8)  		#replace 时间对象写入时区
    >>> d1 = u - l1
    >>> d2 = u - l2
    >>> d1.seconds
    0
    ...
    def __init__(self, utc):
        ...
        if mt:
            ...
            if minus:
                h, m = (-h), (-m)
            self._utcoffset = datetime.timedelta(hours=h, minutes=m)			#timedelta			将天，小时，分钟，秒转换成对应的datetime格式表示
...
class Request(object):
    ...
    def _parse_input(self):
        ...
        """
        ##wsgi.input字段中内容解析,具体看_convert函数操作:传入MiniFieldStorage(u'a', u'1')对象，返回MiniFieldStorage(u'a', u'1').value,即为 u'1'
        def _convert(item):
        ...
        ###fs = FieldStorage(None, None, [MiniFieldStorage(u'a', u'1'), MiniFieldStorage(u'b', u'M M'), MiniFieldStorage(u'c', u'ABC'), MiniFieldStorage(u'c', u'XYZ'), MiniFieldStorage(u'e', u'')])
        fs = cgi.FieldStorage(fp=self._environ['wsgi.input'], environ=self._environ, keep_blank_values=True)  ###cgi.FieldStorage 将urlencoded对象('a=1&b=M%20M&c=ABC&c=XYZ&e=')，解析成类字典的形式[(x,y),(x,y),(x,y)]
        inputs = dict()
        for key in fs:
    ...
    ##字符串被当作url提交时会被自动进行url编码处理，在Python里也有个urllib.urlencode 的方法，可以很方便的把字典形式的参数进行url编码。可是在分析httpheaders的传输信息时，很多已经被url编码的字符串，不是我们这些菜鸟一眼能看出来的，于是乎，urllib.unquote()
    @property
    def path_info(self):
        """
...
#####'^\\/path\\/to\\/(?P<file>[^\\/]+)$'中(?P<file>...)部分表示命名group,在匹配结果的时候可以直接用对应名称直接得到匹配值
####即 re.search('^\\/path\\/to\\/(?P<file>[^\\/]+)$','/path/to/:file').group('file')==re.search('^\\/path\\/to\\/(?P<file>[^\\/]+)$','/path/to/:file').group(1)

###类似'\\/'形式的正则表达式，在字符串中，两个反斜杠被解释为一个反斜杠，然后在作为正则表达式，\/ 则被正则表达式引擎解释为 /，应此在使用正则表达式时尽量使用r''元字符来表示
def _build_regex(path):
    ...
    var_list = []
    is_var = False
##split会按照正则表达式所匹配到的对象作为分割点，如果正则表达式中有group,则以正则表达式来分割的结果(list)中包含group所匹配的内容
###例： re.compile(r'(a)b(c)').split('123abc123') = >  ['123', 'a', 'c', '123']
...
class StaticFileRoute(object):
    ...
    def __call__(self, *args):
        ...
        if not os.path.isfile(fpath):
            raise HttpError.notfound()
        ### os.path.splitext 分割文件名和文件后缀名
        fext = os.path.splitext(fpath)[1]
        ## mimetypes是Python自带的标准库，可以根据文件后缀直接得到文件的MIME类型,mimetypes.types_map 用字典存储了所有文件后缀对应的MIME类型
