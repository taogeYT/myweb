...
async def test(request):
    users = [{'password': '123123', 'name': 'yatao', 'created_at': None, 'id': 1}, {
        'password': '123123', 'name': 'lyt', 'created_at': None, 'id': 2}]  # await Test.find_all()
    # print(users)
...
async def api_register_user(*, email, name, passwd):
    ...
    user = User(id=uuid, email=email, name=name.strip(), passwd=hashlib.sha1(
        sha1_passwd.encode('utf-8')).hexdigest(), image='')  # 摘要算法输入的是字节串
    await user.save()
    # make session cookie:
    r = web.Response()
    # 如果您在cookie中设置了HttpOnly属性，那么通过js脚本将无法读取到cookie信息，这样能有效的防止XSS攻击
    r.set_cookie(COOKIE_NAME, user2cookie(
        user, 300), max_age=300, httponly=True)
    user.passwd = '******'
    r.content_type = 'application/json'
    r.body = json.dumps(user, ensure_ascii=False).encode(
        'utf-8')  # ensure_ascii 表示转换为json格式时字符串内容全部当作ascii转换
    return r
