...
log_format = '%(levelname)s %(filename)10s %(message)s'
...
def init_jinja2(app, **kw):
	...
	filters = kw.get('filters', None)#过滤器作用是对html文档中的要加载的变量做进一步处理后在显示，而不是直接显示变量的值，该时间过滤器作用是把时间戳转换成多久前显示
	if filters is not None:
		for name, f in filters.items():
			env.filters[name] = f
	app.__templating__ = env#前面已经把jinja2的环境配置都赋值给env了，这里再把env存入app的属性中方便调用，这样app就知道要到哪儿去找模板，怎么解析模板。

#middle函数
async def logger_factory(app, handler):
	async def logger(request):
		print(request.query_string)
		logging.info('Server receviced a Request: %s %s' % (request.method, request.path))
...
async def auth_factory(app,handler):
	async def auth(request):
		logging.info('start user authentication: %s %s'%(request.method,request.path))
	...
	async def response(request):
		...
		if isinstance(r, dict):
			...
			if template is None:
				...
				r['__user__'] = request.__user__    #通过auth_factory中返回的request.__user__值来确定用户是否登录成功状态
...
async def init(loop):
	rs = await orm.create_pool(loop=loop,host='localhost', port=3306, user='root', password='password', db='awesome')
	print(rs)
